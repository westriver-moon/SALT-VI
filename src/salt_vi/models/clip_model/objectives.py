...
def _identity_prototypes(features, labels):
    """Return normalized per-identity prototypes and inverse class indices."""
    features = F.normalize(features, dim=-1)
    unique_labels, inverse = torch.unique(labels, sorted=True, return_inverse=True)
    prototypes = []
    for class_index in range(unique_labels.numel()):
        prototypes.append(features[inverse == class_index].mean(dim=0))
    return F.normalize(torch.stack(prototypes, dim=0), dim=-1), inverse


def imta_prototype_loss(text_rgb, text_ir, visual_rgb, visual_ir, labels, temperature=0.07):
    """Classify both text views against detached shared visual ID prototypes."""
    shared_visual = F.normalize((visual_rgb.detach() + visual_ir.detach()) * 0.5, dim=-1)
    prototypes, targets = _identity_prototypes(shared_visual, labels)
    text = F.normalize(torch.cat((text_rgb, text_ir), dim=0), dim=-1)
    logits = text @ prototypes.t() / float(temperature)
    return F.cross_entropy(logits, torch.cat((targets, targets), dim=0))


def imta_dual_text_supcon_loss(text_rgb, text_ir, labels, temperature=0.07):
    """Supervised contrastive alignment of RGB/IR captions with self pairs masked."""
    features = F.normalize(torch.cat((text_rgb, text_ir), dim=0), dim=-1)
    ...
    if torch.any(positive_count == 0):
        raise ValueError("IMTA dual-text SupCon requires at least two views per identity")
    return -((log_prob * positive_mask).sum(dim=1) / positive_count).mean()


def imta_relation_loss(text_rgb, text_ir, visual_rgb, visual_ir, temperature=0.07):
    """Distill the row-wise visual neighborhood distribution into the text space."""
    visual = F.normalize((visual_rgb.detach() + visual_ir.detach()) * 0.5, dim=-1)
    text = F.normalize((text_rgb + text_ir) * 0.5, dim=-1)
    if visual.size(0) < 2:
        return visual.sum() * 0.0
    diagonal = torch.eye(visual.size(0), dtype=torch.bool, device=visual.device)
    visual_logits = (visual @ visual.t() / float(temperature)).masked_fill(diagonal, -1e4)
    text_logits = (text @ text.t() / float(temperature)).masked_fill(diagonal, -1e4)
    teacher = F.softmax(visual_logits, dim=1)
    return F.kl_div(F.log_softmax(text_logits, dim=1), teacher, reduction="batchmean")

def orthogonal_loss(ir_feats, t_feats, text_filter_feats):
    """
    Compute Orthogonal Loss
    """
    ir_feats = F.normalize(ir_feats, p=2, dim=1)
    redirect_feats = F.normalize(t_feats - text_filter_feats, p=2, dim=1)

    # cosine similarity as logits
    logits_per_image = ir_feats @ redirect_feats.t()
    loss = F.l1_loss(logits_per_image, torch.zeros_like(logits_per_image))
    return loss

def orthogonal_loss2(ir_feats, t_feats, text_filter_feats):
    """
    Compute Orthogonal Loss
    """
    ir_feats = F.normalize(ir_feats, p=2, dim=1)
    ...
    # cosine similarity as logits
    logits_per_image = ir_feats @ redirect_feats.t()
    loss = F.l1_loss(logits_per_image, torch.zeros_like(logits_per_image),reduction='sum')
    return loss


# i2r, t2r cosine similarity's alignment loss
def compute_sim_align(rgb_features, text_features, ir_features, logit_scale, loss_name):
    """
    Compute similarity alignment loss
    """ 
    # normalized features
    rgb_norm = rgb_features / rgb_features.norm(dim=-1, keepdim=True)
    text_norm = text_features / text_features.norm(dim=-1, keepdim=True)
    ir_norm = ir_features / ir_features.norm(dim=-1, keepdim=True)
    # fusion_norm = fusion_features / fusion_features.norm(dim=-1, keepdim=True)

    # cosine similarity as logits r2t # text
    logits_per_image0 = logit_scale * rgb_norm @ text_norm.t()
    logits_per_text0 = logits_per_image0.t()

    # cosine similarity as logits r2i # ir
    logits_per_image1 = logit_scale * rgb_norm @ ir_norm.t()
    logits_per_text1 = logits_per_image1.t()

    # # cosine similarity as logits
    # logits_per_image = logit_scale * rgb_norm @ fusion_norm.t()
    # logits_per_text = logits_per_image.t()
    if 'itd' in loss_name:
        loss = CoRefineLoss(logits_per_text1, logits_per_text0.detach())
    if 'tid' in loss_name:
        loss = CoRefineLoss(logits_per_text0, logits_per_text1.detach())
        
    return loss


def compute_feat_kldiv(feat1, feat2):
    """
    Compute KL divergence between two feature maps
    """
    feat1 = F.normalize(feat1, p=2, dim=1)
    feat2 = F.normalize(feat2, p=2, dim=1)
    return F.kl_div(F.log_softmax(feat1, dim=1), F.softmax(feat2, dim=1), reduction='batchmean')
