...
class LRSchedulerWithWarmup(_LRScheduler):
    def __init__(
        ...
        if mode != "step" and total_epochs <= warmup_epochs:
            raise ValueError(
                "total_epochs must be greater than warmup_epochs for "
                f"{mode!r} scheduling, got {total_epochs} and {warmup_epochs}"
            )
    ...
    def get_lr(self):
        ...
        epoch_ratio = min(1.0, max(0.0, epoch_ratio))
