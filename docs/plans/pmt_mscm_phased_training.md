...
> **历史设计说明（重要）**：本文记录的是早期“epoch 6 后切换、以 QCT 为主”的 recipe 设计，保留用于解释当时的动机和代码边界。它不是 2026-08-21/22 H1–H4 混合损失实验的结果说明。当前 H1–H4 的实际终端权重、渐进因子、加速设置和解析配置以 `configs/stage_a/plugins/hybrid_loss/common.yaml` 及各 H1–H4 YAML、归档中的 `configs.yaml` 为准；当前结果见 `docs/status/experiment_status_20260822.md`。
