#!/usr/bin/env python3
"""Dynamic one-job-per-idle-GPU scheduler for A3-E4 SR Stage-2 tuning."""
...
DEFAULT_PLAN = REPO_ROOT / "configs/metric_boost/a3_e4_hpt_stage2.yaml"
MAIN = REPO_ROOT / "scripts" / "train.py"
CONFIRM_TOKEN = "I_UNDERSTAND_A3_E4_HPT_STAGE2_WILL_START"
...
def load_plan(path: Path = DEFAULT_PLAN) -> Dict[str, Any]:
    ...
    if len(identifiers) != 8 or len(identifiers) != len(set(identifiers)):
        raise ValueError("Stage 2 must declare exactly 8 new experiment IDs")
    expected = {
        f"A3E4-S2-L{lr}-W{weight}"
        for lr in ("025", "050", "075")
        for weight in ("075", "100", "125")
    } - {"A3E4-S2-L025-W100"}
    if set(identifiers) != expected:
        raise ValueError(f"Stage-2 grid mismatch: expected {sorted(expected)}, got {sorted(identifiers)}")
    return plan
...
def enforce_protocol(runtime: Mapping[str, Any]) -> None:
    ...
    if mismatches:
        raise ValueError(f"A3-E4 Stage-2 protocol violation: {mismatches}")
...
def shared_input_fingerprint(
    ...
    if missing:
        raise FileNotFoundError(f"Required A3-E4 Stage-2 input files are missing: {missing}")
    payload = {
        "created_at": utc_now(),
...
def materialize(
    ...
    design = (
        ...
        "This is one point in the A3-E4 SR Stage-2 lr_txt x cross-modal-hard grid. "
        "Every point starts from the exact same E4 epoch-21 checkpoint and runs on one physical GPU.\n"
    )
...
class Scheduler:
    def __init__(
        ...
        if not warm_start.is_file() or sha256_file(warm_start) != str(self.plan["warm_start_sha256"]):
            raise RuntimeError("A3-E4 warm-start checkpoint is missing or its SHA-256 changed")
        self.baseline = probe
...
def main() -> None:
    ...
    if args.preflight:
        ...
        payload = {
            ...
            "idle_gpu_ids": idle_from_inventory(gpu_inventory()),
        }
    ...
    with lock_path.open("a+", encoding="utf-8") as lock:
        try:
            ...
            raise RuntimeError("Another A3-E4 Stage-2 scheduler is already active") from exc
