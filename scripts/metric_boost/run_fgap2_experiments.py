#!/usr/bin/env python
"""Provenance-gated, lease-safe runner for the three FGAP2 experiments."""
...
DEFAULT_PLAN = REPO_ROOT / "configs/metric_boost/fgap2_experiments.yaml"
...
def load_plan(path: Path = DEFAULT_PLAN):
    ...
    if len(rows) != 3 or len({row["id"] for row in rows}) != 3:
        raise ValueError("FGAP2 plan must contain exactly three unique experiments")
    return rows
...
def _materialize_provenance(spec, runtime_path: Path, command, checkpoint, gpu_id: int, plan_path: Path):
    ...
    design = (
        ...
        "Declared baseline: TRAIN-3-H1. All six cross-modal hard-loss pairs remain equal-weight. "
        "P1 adds modality-specific BN, P2 freezes QBN running statistics from zero-based epoch 5 while "
        "keeping affine parameters trainable, and P3 adds precomputed LLM caption replacement at probability 0.5.\n\n"
        "Controlled variables: pa=0.5, id+cross_modal_hard, frozen visual backbone, RGB_IR_Text, "
        "Fusion evaluation, 288x144 cls pooling, SYSU all-search/single-shot/10 trials, seed and E4 initialization.\n\n"
        "Selection rule: independently track best Rank-1, best mAP and best mINP. This is exploratory "
        "test-set-tuned selection; interaction evidence using legacy PAIRWISE-1-hard-llm05 is historical only.\n\n"
    ...
    manifest = {
        ...
        "experiment_id": spec["id"], "stage": spec["stage"], "group": "FGAP2", "attempt": 1,
        ...
        "code_change_summary": "Committed FGAP2 QBN freeze, three-metric checkpoint selection, safe GPU process exclusion, event and provenance implementation",
...
def _summarize_completed_run(status_path: Path, events_path: Path):
    status = read_json(status_path, {})
    events = _read_events(events_path)
    evals = [event for event in events if event.get("event_type") == "eval_epoch"]
    trains = [event for event in events if event.get("event_type") == "train_epoch"]
    if not evals:
        raise RuntimeError("Training completed without eval_epoch events")
    final_paths = {}
    for event in evals:
        final_paths.update(event.get("checkpoint_paths", {}))
    best_by_metric = {}
    for metric in ("Rank-1", "mAP", "mINP"):
        best = max(evals, key=lambda event: (float(event["metrics"][metric]), -int(event["epoch"])))
        path = final_paths.get(metric)
        if not path or not Path(path).is_file():
            raise RuntimeError(f"Missing retained best-{metric} checkpoint: {path}")
        best_by_metric[metric] = {
            "value": float(best["metrics"][metric]),
            "epoch": int(best["epoch"]),
            "checkpoint": str(Path(path).resolve()),
            "metrics_at_epoch": {key: float(value) for key, value in best["metrics"].items()},
        }
    status.update({
        "status": "succeeded",
        "Rank-1": best_by_metric["Rank-1"]["value"],
        "mAP": best_by_metric["mAP"]["value"],
        "mINP": best_by_metric["mINP"]["value"],
        "best_epoch": best_by_metric["Rank-1"]["epoch"],
        "checkpoint": best_by_metric["Rank-1"]["checkpoint"],
        "best_by_metric": best_by_metric,
        "last_completed_epoch": max(int(event["epoch"]) for event in trains),
        "updated_at": utc_now(),
    })
    atomic_write_json(status_path, status)
    emitted = set()
    for metric, row in best_by_metric.items():
        identity = (row["checkpoint"], row["epoch"])
        digest = sha256_file(Path(row["checkpoint"]))
        _append_event(events_path, status["experiment_id"], "checkpoint_saved", epoch=row["epoch"],
                      path=row["checkpoint"], sha256=digest, is_best=True, selection_metric=metric,
                      shared_physical_checkpoint=identity in emitted)
        emitted.add(identity)
    return status


def _maybe_finalize_plan(plan_path: Path):
    lock_path = PLAN_ROOT / "fgap2-finalizer.lock"
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            return False
        statuses = [read_json(experiment_status_path(spec["id"]), {}) for spec in load_plan(plan_path)]
        if not all(status.get("status") == "succeeded" for status in statuses):
            return False
        subprocess.run([sys.executable, str(SCRIPT_DIR / "plot_fgap2_results.py"), "--plan", str(plan_path)],
...
def run_one(spec, plan_path: Path, checkpoint_plan: Path, poll_seconds: float):
    ...
    try:
        ...
        atomic_write_json(status_path, status)
        status = _summarize_completed_run(status_path, events_path)
        _append_event(events_path, spec["id"], "run_finished", status=status.get("status"), return_code=0,
                      completion_reason="completed",
                      final_metrics={key: status.get(key) for key in ("Rank-1", "mAP", "mINP")},
                      selected_checkpoint=status.get("checkpoint"),
                      selected_checkpoints=status.get("best_by_metric"))
        _maybe_finalize_plan(plan_path)
        return status
