...
def main() -> None:
    ...
    for group in groups:
        ...
        if not source_is_complete(group, output_root):
    ...
    if len(groups) != args.expected_sources:
        errors.append(f"record_count:{len(groups)}!=expected:{args.expected_sources}")
    summary = {
        "expected_sources": args.expected_sources,
        "record_sources": len(groups),
        "valid_views": view_count,
        "expected_views": args.expected_sources * 5,
        "errors": errors[:1000],
        "error_count": len(errors),
        "complete": not errors and view_count == args.expected_sources * 5,
    }
    ...
    if not summary["complete"]:
        raise SystemExit(1)
    consolidate_manifest(output_root, expected_sources=args.expected_sources)
