...
def main() -> None:
    ...
    for group in groups:
        ...
        expected_indices = list(range(config.views_per_source))
        if len(group) != config.views_per_source or [task.view_index for task in group] != expected_indices:
            errors.append(f"view_contract:{source_key}")
            continue
        ...
        if len(metadata.get("views", [])) != config.views_per_source:
            errors.append(f"view_count:{source_key}")
            continue
        try:
            validate_geometry(metadata, config)
        except Exception as error:
            errors.append(f"geometry:{source_key}:{error}")
            continue
