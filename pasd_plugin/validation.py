...
def validate_geometry(geometry: dict, config: PluginConfig) -> None:
    ...
    if geometry.get("mode") != "person_fit_blurred_background":
        raise ValueError(f"unknown geometry mode: {geometry.get('mode')}")
...
def validate_protocol(record: SourceRecord, dataset: str) -> None:
    if dataset == "sysu":
        if set(record.protocol) != {"sysu"} or record.protocol["sysu"].get("split") not in {"train", "val", "test"}:
        ...
        if membership.get("split") not in {"train", "test"} or membership.get("label") != record.source_label:
...
def validate_manifest(config: PluginConfig, records: list[SourceRecord]) -> list[dict]:
    ...
    if not manifest_path.is_file() or not summary_path.is_file():
        return [{"source_key": None, "error": "missing manifest.jsonl or manifest.json"}]
    try:
    ...
    for row in rows:
        ...
        try:
            ...
            if any(row.get(field) != value for field, value in expected_payload.items()):
                raise ValueError("manifest record differs from records contract")
            marker = validate_record(config, record)
            for field in ("input_sha256", "build_sha256", "source_contract_sha256", "artifact", "geometry"):
    ...
    errors.extend({"source_key": key, "error": "source absent from manifest"} for key in missing)

    digest = sha256_file(manifest_path)
    if summary.get("dataset") != config.dataset:
        errors.append({"source_key": None, "error": "manifest dataset mismatch"})
    if summary.get("expected_source_count") != len(records) or summary.get("source_count") != len(rows):
    ...
    if summary.get("build_sha256") != config.build_sha256:
        errors.append({"source_key": None, "error": "manifest build fingerprint mismatch"})
