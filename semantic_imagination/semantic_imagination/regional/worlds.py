...
from .schema import Assignment, Candidate, Region, World
...
def _caption(assignments: list[Assignment]) -> str:
    details = [
        ...
        if assignment.state not in SENTINEL_STATES
    ]
    if not details:
        return "faithful pedestrian restoration with no additional uncertain detail"
    return "faithful pedestrian restoration; preserve identity and pose; regional details: " + "; ".join(details)
...
def build_worlds(
    ...
    samples: list[dict[str, Candidate]],
    *,
    max_worlds: int,
    seed: int,
) -> list[World]:
    ...
    for region in regions:
        ...
        region.u_qwen = regional_entropy(
    ...
    accepted: list[tuple[tuple[tuple[str, str, str], ...], dict[str, Candidate], dict]] = []
